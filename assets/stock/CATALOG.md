# ಊರ್ಮನಿ ಸುದ್ದಿ — Evergreen Editorial Stock Library (ಸಾರ್ವತ್ರಿಕ ಸಾಂದರ್ಭಿಕ ಚಿತ್ರಕೋಶ)

> **Location**: `assets/stock/`  
> **Licence**: `own`  
> **Credit**: `AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ`  
> **Nature**: `ai` (ಎಐ ರಚಿತ ಚಿತ್ರ) — these frames were generated. Never reuse them as `representative`.  

This library contains **only truly generic, evergreen newsroom stock visuals** that can be reused across any future coastal Karnataka news report whenever specific spot photography is unavailable.

---

### 1. `police_dog_squad_investigation.jpg`
- **Use in Any**: Murder, burglary, robbery, missing person search, forensic tracking, or general police investigation news.
- **Visual**: Police sniffer dog handler and forensic team tracking clues on site.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/police_dog_squad_investigation.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ಘಟನಾ ಸ್ಥಳದಲ್ಲಿ ತನಿಖೆ ನಡೆಸುತ್ತಿರುವ ಪೊಲೀಸ್ ಶ್ವಾನ ದಳ (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```

---

### 2. `police_cctv_surveillance_monitoring.jpg`
- **Use in Any**: CCTV footage review, cyber crime, hit-and-run tracking, thief identification, or city surveillance news.
- **Visual**: Police and investigation team analyzing surveillance video displays in a monitoring room.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/police_cctv_surveillance_monitoring.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ಕಣ್ಗಾವಲು ಕ್ಯಾಮೆರಾ ದೃಶ್ಯಾವಳಿಗಳನ್ನು ಪರಿಶೀಲಿಸುತ್ತಿರುವ ತನಿಖಾ ತಂಡ (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```

---

### 3. `traffic_police_highway_patrol.jpg`
- **Use in Any**: Traffic checking, helmet/seatbelt drives, drunk driving, road safety advisories, or highway incident inspection.
- **Visual**: Coastal traffic police officers on duty on the highway.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/traffic_police_highway_patrol.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ಹೆದ್ದಾರಿಯಲ್ಲಿ ತಪಾಸಣೆ ನಡೆಸುತ್ತಿರುವ ಸಂಚಾರ ಪೊಲೀಸರು (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```

---

### 4. `coastal_nh66_highway_traffic.jpg`
- **Use in Any**: National Highway 66 news, bus fare hikes, holiday traffic jams, monsoon driving advisories, or coastal transport stories.
- **Visual**: Four-lane coastal national highway with moving vehicles bordered by palm trees.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/coastal_nh66_highway_traffic.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ಕರಾವಳಿ ರಾಷ್ಟ್ರೀಯ ಹೆದ್ದಾರಿಯಲ್ಲಿ ವಾಹನ ಸಂಚಾರ (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```

---

### 5. `taluk_revenue_office_files.jpg`
- **Use in Any**: Government orders, Lokayukta raids, land record/RTC issues, Gram Panchayat administration, or public grievance reports.
- **Visual**: Official government office desk with revenue files, folders, and official stamps.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/taluk_revenue_office_files.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ಕಂದಾಯ ಇಲಾಖಾ ಕಚೇರಿ ಹಾಗೂ ಸರಕಾರಿ ಕಡತಗಳು (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```

---

### 6. `coastal_fishing_harbour_docks.jpg`
- **Use in Any**: Fishing ban/season start, sea storm warnings for fishermen, boat diesel subsidies, fish prices, or harbour dock news.
- **Visual**: Coastal fishing harbour with mechanized trawlers, fishing boats, and fishermen at the wharf.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/coastal_fishing_harbour_docks.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ಕರಾವಳಿ ಮೀನುಗಾರಿಕಾ ಬಂದರಿನ ದಕ್ಕೆ (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```

---

### 7. `rural_weekly_market_farmers_shandy.jpg`
- **Use in Any**: Vegetable price rise/drop, inflation, rural trade, farmers market news, APMC updates, or rural local economy.
- **Visual**: Bustling rural weekly shandy (ವಾರದ ಸಂತೆ) with fresh vegetables, stalls, and local buyers.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/rural_weekly_market_farmers_shandy.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ಗ್ರಾಮೀಣ ವಾರದ ಸಂತೆ ಹಾಗೂ ತರಕಾರಿ ಮಾರುಕಟ್ಟೆ (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```

---

### 8. `civic_stray_dogs_street_menace.jpg`
- **Use in Any**: Stray dog menace, municipal animal birth control (ABC) drives, rabies vaccination campaigns, pedestrian safety, or urban civic negligence news.
- **Visual**: Coastal town commercial street with wandering stray dogs near local shopfronts.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/civic_stray_dogs_street_menace.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ನಗರದ ರಸ್ತೆಯಲ್ಲಿ ಬೀಡುಬಿಟ್ಟಿರುವ ಬೀದಿನಾಯಿಗಳು (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```

---

### 9. `coastal_court_complex_judiciary.jpg`
- **Use in Any**: Court verdicts, criminal sentencing, judicial hearings, bail pleas, police custody orders, or legal bar association updates.
- **Visual**: Coastal Karnataka court complex building with lawyers in black coats and litigants in the courtyard.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/coastal_court_complex_judiciary.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ಜಿಲ್ಲಾ ನ್ಯಾಯಾಲಯ ಸಂಕೀರ್ಣದ ಆವರಣ (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```

---

### 10. `temple_ganahoma_vedic_ritual.jpg`
- **Use in Any**: Temple Ganahoma, Yajna rituals, Brahmakalashotsava religious events, Vedic poojas, or coastal temple festivals.
- **Visual**: Sacred Ganahoma fire altar with priests performing Vedic oblations in a traditional coastal stone temple courtyard.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/temple_ganahoma_vedic_ritual.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ದೇವಸ್ಥಾನದಲ್ಲಿ ನಡೆಯುತ್ತಿರುವ ಮಹಾ ಗಣಹೋಮ ಧಾರ್ಮಿಕ ವಿಧಿವಿಧಾನ (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```

---

### 11. `coastal_badagutittu_yakshagana_performance.jpg`
- **Use in Any**: Yakshagana mela tours, cultural performances, temple Bayalata, Rangasthala artist news, or coastal folk art festivals.
- **Visual**: Traditional Badagu Tittu Yakshagana artiste in majestic Kedage Kireeta and colourful costume performing on stage.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/coastal_badagutittu_yakshagana_performance.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ಕರಾವಳಿಯ ಗಂಡುಕಲೆ ಬಡಗುತಿಟ್ಟು ಯಕ್ಷಗಾನ ಬಯಲಾಟ ಪ್ರದರ್ಶನ (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```

---

### 12. `coastal_shiva_temple_linga_darshana.jpg`
- **Use in Any**: Shiva temple festivals, Maha Shivaratri, Karthika Deepotsava, Rudrabhisheka, or coastal temple sanctum darshana news.
- **Visual**: Sacred stone Shiva Linga adorned with Tripundra and Bilva leaves, with priest waving multi-tiered camphor aarti lamp in sanctum.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/coastal_shiva_temple_linga_darshana.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ದೇವಸ್ಥಾನದ ಗರ್ಭಗುಡಿಯಲ್ಲಿ ಶ್ರೀ ಶಿವಲಿಂಗ ದರ್ಶನ ಹಾಗೂ ಮಹಾಮಂಗಳಾರತಿ (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```

---

### 13. `ganeshotsava_idol_devotional_pooja.jpg`
- **Use in Any**: Ganeshotsava festival news, public Ganesha idol installations, Vinayaka Chaturthi celebrations, or coastal temple pooja news.
- **Visual**: Decorated Lord Ganesha idol with brass deepas, incense, and floral garlands in sanctum sanctorum.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/ganeshotsava_idol_devotional_pooja.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ಶ್ರೀ ಗಣೇಶೋತ್ಸವದ ಪ್ರಯುಕ್ತ ಪೂಜಿಸಲ್ಪಟ್ಟ ಶ್ರೀ ಮಹಾಗಣಪತಿ ವಿಗ್ರಹ (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```

---

### 14. `coastal_vented_dam_river_water.jpg`
- **Use in Any**: Vented dams, drinking water shortage, river water preservation, monsoon runoff control, irrigation board updates, or coastal river barrages.
- **Visual**: Coastal river flowing toward the estuary with a concrete vented dam and fitted wooden planks, flanked by coconut palms.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/coastal_vented_dam_river_water.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ಜಿಲ್ಲೆಯ ನದಿಗೆ ಅಡ್ಡಲಾಗಿ ನಿರ್ಮಿಸಲಾಗಿರುವ ಕಿಂಡಿ ಅಣೆಕಟ್ಟು ಹಾಗೂ ಹಲಗೆಗಳು (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```

---

### 15. `wildlife_leopard_trap_forest_cage.jpg`
- **Use in Any**: Leopard attacks, wild animal intrusion in villages, Forest Department trap cages, wildlife tracking, or human-wildlife conflict news.
- **Visual**: Sturdy metal trap cage installed by the Forest Department inside an arecanut plantation with warning sign and forest guards.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/wildlife_leopard_trap_forest_cage.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ಗ್ರಾಮಾಂತರ ಭಾಗದಲ್ಲಿ ಚಿರತೆ ಸೆರೆಹಿಡಿಯಲು ಅರಣ್ಯ ಇಲಾಖೆ ಇರಿಸಿರುವ ಬೋನು (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```

---

### 16. `gauri_ganesha_festival_pooja_altar.jpg`
- **Use in Any**: Swarna Gauri Vrata, Ganeshotsava festival greetings, coastal home/temple pooja mantapa rituals, modaka prasad, or festival bulletins.
- **Visual**: Traditional wooden carved mantapa with beautifully adorned Sri Swarna Gauri and Sri Ganesha idols, brass deepas, modakas, and coconut offerings.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/gauri_ganesha_festival_pooja_altar.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ಸ್ವರ್ಣ ಗೌರಿ ಹಾಗೂ ಶ್ರೀ ಮಹಾಗಣಪತಿ ಪೂಜಾ ಮಂಟಪ ಮತ್ತು ನೈವೇದ್ಯ (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```



---

### 17. `coastal_idol_maker_ganesha_craftsman.jpg`
- **Use in Any**: Ganesh Chaturthi idol-making season, traditional clay artisan (ಕುಂಬಾರ) livelihood stories, eco-friendly idol campaigns, or Kumbara community features.
- **Visual**: An elderly artisan hand-finishing a clay Ganesha idol at a potter's wheel, surrounded by earthen pots in a traditional workshop.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/coastal_idol_maker_ganesha_craftsman.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ಗಣೇಶ ವಿಗ್ರಹ ರೂಪಿಸುತ್ತಿರುವ ಕುಂಬಾರ ಕಲಾವಿದ (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```

---

### 18. `coastal_ghats_monsoon_homestead.jpg`
- **Use in Any**: Western Ghats monsoon rainfall, Karkala/Hebri hill-taluk weather stories, rural connectivity, arecanut/farm-belt features, or landslide-risk advisories.
- **Visual**: A tiled-roof homestead amid areca and coconut palms, with mist-covered Western Ghats hills in the background after rain.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/coastal_ghats_monsoon_homestead.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ಮಲೆನಾಡು ಗಡಿಯ ಮಳೆಗಾಲದ ಗ್ರಾಮೀಣ ಮನೆ ಮತ್ತು ಬೆಟ್ಟಗಳು (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```

---

### 19. `ambulance_emergency_response_town.jpg`
- **Use in Any**: Road accidents, 108 ambulance response time stories, medical emergency advisories, or trauma-care access features.
- **Visual**: A 108 government ambulance moving through a busy coastal town street lined with shops and auto-rickshaws.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/ambulance_emergency_response_town.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ಪಟ್ಟಣದ ರಸ್ತೆಯಲ್ಲಿ ಧಾವಿಸುತ್ತಿರುವ 108 ಆರೋಗ್ಯ ಕವಚ ಆಂಬ್ಯುಲೆನ್ಸ್ (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```

---

### 20. `coastal_event_site_police_inspection.jpg`
- **Use in Any**: Festival/event security arrangements, VIP visit preparation, beach event permissions, or officials' pre-event site inspections.
- **Visual**: Police officers and civic officials reviewing site plans at a beachside venue being set up for a public event.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/coastal_event_site_police_inspection.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ಕಡಲತೀರದ ಕಾರ್ಯಕ್ರಮ ಸ್ಥಳದಲ್ಲಿ ಪೊಲೀಸ್ ಹಾಗೂ ಅಧಿಕಾರಿಗಳ ಪೂರ್ವಭಾವಿ ಪರಿಶೀಲನೆ (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```

---

### 21. `forest_dept_timber_seizure_inspection.jpg`
- **Use in Any**: Illegal logging busts, forest offence cases, timber seizure news, or Western Ghats conservation enforcement stories.
- **Visual**: Forest Department officials inspecting and tagging a stack of seized timber logs at a forest checkpost.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/forest_dept_timber_seizure_inspection.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ವಶಪಡಿಸಿಕೊಂಡ ಮರದ ದಿಮ್ಮಿಗಳನ್ನು ಪರಿಶೀಲಿಸುತ್ತಿರುವ ಅರಣ್ಯ ಇಲಾಖೆ ಅಧಿಕಾರಿಗಳು (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```

---

### 22. `education_scholarship_certificate_ceremony.jpg`
- **Use in Any**: Scholarship distribution, college graduation/merit-award functions, or education department felicitation events.
- **Visual**: Students seated with framed certificates at a formal scholarship distribution ceremony, dignitaries on stage behind.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/education_scholarship_certificate_ceremony.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ವಿದ್ಯಾರ್ಥಿವೇತನ ವಿತರಣಾ ಸಮಾರಂಭ (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```

---

### 23. `civic_flooded_road_villagers_complaint.jpg`
- **Use in Any**: Drainage/waterlogging complaints, monsoon flooding of rural roads, civic negligence stories, or panchayat infrastructure grievances.
- **Visual**: Villagers pointing at a flooded, muddy rural road beside paddy fields after heavy rain.
- **Reference**:
```json
"photo": {
  "path": "assets/stock/civic_flooded_road_villagers_complaint.jpg",
  "nature": "ai",
  "credit": "AI ಚಿತ್ರ — ಊರ್ಮನಿ ಸುದ್ದಿ",
  "licence": "own",
  "caption": "ಮಳೆಗಾಲದಲ್ಲಿ ಜಲಾವೃತಗೊಂಡ ಗ್ರಾಮೀಣ ರಸ್ತೆ ಬಗ್ಗೆ ಗ್ರಾಮಸ್ಥರ ಆಕ್ಷೇಪ (ಎಐ ರಚಿತ ಚಿತ್ರ)"
}
```
